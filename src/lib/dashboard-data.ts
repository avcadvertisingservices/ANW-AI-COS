...
export async function getDashboardData(): Promise<DashboardData> {
  ...
  const [
    ...
    supabase
      .from("knowledge_review_requests")
      .select(
        "id,status,knowledge_entry_id,knowledge_slug,created_at",
      )
      .in("status", [
        "draft",
        "submitted",
...
function countActiveSources(events: SourceEventRow[]): number {
  ...
  for (const event of events) {
    ...
    // The query is newest first, so the first event for each source
    // represents that source's current state.
  ...
  for (const latestEventType of latestEventBySource.values()) {
